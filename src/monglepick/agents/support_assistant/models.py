...
SupportReplyKind = Literal[
    "faq", "partial", "complaint", "out_of_scope", "smalltalk"
]
...
class SupportAssistantState(TypedDict, total=False):
    """
    support_assistant LangGraph State (v3 단순화).
    ...
    - reply      : Solar Pro 가 돌려준 SupportReply 구조화 응답
    - matched_faqs: reply.matched_faq_ids 를 FAQ 메타와 매핑한 축약 리스트 (SSE 발행용)
    - response_text: 최종 본문 (reply.answer 를 response_formatter 가 한 번 더 검증)
